...
умолчанию. MVP: без шифрования (явно зафиксировано в UI Settings — "данные
хранятся в браузере без шифрования").

**[v2] Решение по шифрованию.** Opt-in шифрование at-rest через Web Crypto
(passphrase → PBKDF2 → AES-GCM) сознательно не реализовано: оно неизбежно
требует ввода passphrase пользователем при каждом открытии Finance/Diary
(ключ нельзя хранить рядом с зашифрованными данными — это обесценивает
шифрование), то есть требует его участия на постоянной основе, а не
разового решения при разработке. Если понадобится — реализовать отдельным
заходом, с явным согласием пользователя на этот компромисс UX, включая:
экран разблокировки, миграцию существующих plaintext-записей, и решение по
индексации зашифрованного контента в Search (в открытом виде индексировать
нельзя — придётся либо не индексировать Finance/Diary при выключенном
разблокированном состоянии, либо переиндексировать посессионно после
разблокировки).
